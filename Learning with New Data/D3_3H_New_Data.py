...
New Data Deep Learning Model 3
...
def build_D3():
    ...
    keras.layers.Dense(int((2/3)*col_num), activation=tf.nn.tanh,kernel_regularizer=keras.regularizers.l1(0.01),activity_regularizer=keras.regularizers.l2(0.01)),
    keras.layers.Dense(int((2/3)*col_num), activation=tf.nn.tanh,kernel_regularizer=keras.regularizers.l1(0.01),activity_regularizer=keras.regularizers.l2(0.01)),
    keras.layers.Dense(int((2/3)*col_num), activation=tf.nn.tanh,kernel_regularizer=keras.regularizers.l1(0.01),activity_regularizer=keras.regularizers.l2(0.01)),
...
D3 = build_D3()
D3.fit(X_trains, y_trains)

# Validation Set
y_hat1 = D3.predict(X_valid)
y_hat1 = (y_hat1 > 0.5)
y_hat1 = y_hat1.astype(int)
y_hat1 = pd.DataFrame(y_hat1,columns=['DEFAULT'])
print("D3(valid) RMSE:", np.sqrt(metrics.mean_squared_error(y_valid, y_hat1))) 
print("D3(valid) AUC:",roc_auc_score(y_valid, y_hat1))

# Testing Set
y_hat2 = D3.predict(X_test)
y_hat2 = (y_hat2 > 0.5)
y_hat2 = y_hat2.astype(int)
y_hat2 = pd.DataFrame(y_hat2,columns=['DEFAULT'])
print("D3(test) RMSE:", np.sqrt(metrics.mean_squared_error(y_test, y_hat2))) 
print("D3(test) AUC:",roc_auc_score(y_test, y_hat2))
...
# Validation Set
plot_roc(y_valid, y_hat1,'D3','valid data')

# Testing Set
plot_roc(y_test, y_hat2,'D3','test data')