...
# Level 1
for i in kf.split(X_trains,y_trains):
    model_gbdt = build_model_gbdt(i)
    train_gbdt_pred = pd.concat([train_gbdt_pred,model_gbdt.predict(i[0])])
    train_gbm_pred = pd.concat([train_gbm_pred,model_gbm.predict(i[0])])
    train_en_pred = pd.concat([train_en_pred,model_en.predict(i[0])])
